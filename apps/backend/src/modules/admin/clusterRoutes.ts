 ...
 *   POST  /admin/cluster/test       { id }              live-probe a node's agent (ok + latencyMs)
 *   POST  /admin/cluster/enable     { id, enabled }     toggle a single node's enabled flag
 ...
 *
 *   Node-agent side (slave mode):
 *   GET   /admin/cluster/node/status                     is this server's node agent running?
 *   POST  /admin/cluster/node/start                      spawn the node agent (role from config.cluster.role)
 *   POST  /admin/cluster/node/stop                       stop the node agent
 *   POST  /admin/cluster/node/role     { role }          change role, persist + restart the agent
 *   POST  /admin/cluster/generate-token                  mint a new shared pairing token
 */
export function registerClusterRoutes(
  ...
  router.get('/admin/cluster/overview', async (ctx) => {
    ...
    ctx.json({
      ...
      pathPins: deps.config.cluster.pathPins ?? [],
      nodes,
    });
  }, guard);
  ...
  router.get('/admin/cluster/setup', async (ctx) => {
    ...
    ctx.json({
      ...
      self: {
        ...
        role: c.role ?? 'backend',
        agentUrl: selfUrl,
        serve: `nexus node serve --role=${c.role ?? 'backend'} --port=${c.nodeAgentPort}`,
  ...
  /** Persist master/slave host/port overrides (and path pins) into nexus.runtime.json (validated). */
  router.post('/admin/cluster/setup', async (ctx) => {
    ...
    if (Array.isArray(body.pathPins)) {
      // Validate + normalise: drop empty prefixes/nodeIds; coerce role to the enum.
      const pins = body.pathPins
        .filter((p) => p && typeof p.prefix === 'string' && p.prefix.trim() && typeof p.nodeId === 'string' && p.nodeId.trim())
        .map((p) => ({ prefix: p.prefix.trim(), nodeId: p.nodeId.trim(), ...(p.role ? { role: p.role as 'backend' | 'files' | 'database' | 'ai' } : {}) }));
      patch.cluster = { ...(patch.cluster ?? {}), pathPins: pins };
    }
    try {
      ...
      // Apply pathPins live to the LB so routing changes without an LB restart.
      if (Array.isArray(body.pathPins)) {
        const pins = (merged.cluster?.pathPins ?? []).map((p) => ({ prefix: p.prefix, nodeId: p.nodeId, ...(p.role ? { role: p.role } : {}) }));
        deps.config.cluster.pathPins = pins as never;
        manager.setPathPins(pins);
      }
      ctx.json({ ok: true, note: 'master/serve overrides saved — restart the cluster/node processes to apply (path pins apply live)' });
    } catch (err) {
  ...
  router.get('/admin/cluster/node/status', async (ctx) => {
    ...
    ctx.json({
      ...
      nodeId: nodeIdFor(deps.root, role as never),
      role,
      port: c.nodeAgentPort,
  ...
  router.post('/admin/cluster/node/start', async (ctx) => {
    ...
    if (nodeAgentProcess?.exitCode === null && !nodeAgentProcess.killed) {
      ctx.json({ ok: true, running: true, pid: nodeAgentProcess.pid, port: c.nodeAgentPort, role });
      return;
    }
    try {
      const cli = resolve(deps.root, 'bin', 'nexus.js');
      const child = spawn(process.execPath, [cli, 'node', 'serve', `--role=${role}`, `--port=${c.nodeAgentPort}`], {
        cwd: deps.root,
        env: process.env,
      ...
      ctx.json({ ok: true, running: true, pid: child.pid, port: c.nodeAgentPort, role, token: c.token });
    } catch (err) {
      ctx.json({ error: (err as Error).message }, 400);
    }
  }, guard);
  ...
  /** Change this node's cluster role: persist `cluster.role` into
   *  nexus.runtime.json, mutate the in-memory config, then restart the node
   *  agent child with the new role. The node id is derived from the role, so
   *  changing it means the node must be re-linked on the master. */
  router.post('/admin/cluster/node/role', async (ctx) => {
    const { role } = (ctx.body ?? {}) as { role?: string };
    const allowed = ['backend', 'files', 'database', 'ai'];
    if (!role || !allowed.includes(role)) {
      ctx.json({ error: `role is required (one of: ${allowed.join(', ')})` }, 400);
      return;
    }
    try {
      // Persist into nexus.runtime.json (merge with existing overrides).
      const existing = await readRuntime(deps.root);
      const merged = mergeRuntime(existing, { cluster: { role: role as never } });
      await writeRuntime(deps.root, merged);
      deps.config.cluster.role = role as never;

      // Stop the running agent (if any), then respawn with the new role.
      if (nodeAgentProcess && nodeAgentProcess.exitCode === null && !nodeAgentProcess.killed) {
        try {
          if (process.platform === 'win32' && nodeAgentProcess.pid) {
            execFile('taskkill', ['/PID', String(nodeAgentProcess.pid), '/T', '/F'], { windowsHide: true }, () => undefined);
          } else {
      ...
      ctx.json({
        ...
        nodeId: nodeIdFor(deps.root, role as never),
        agentUrl: `http://127.0.0.1:${c.nodeAgentPort}`,
        note: 'role changed — the node id changed; re-link this node on the master if it was already linked',
      });
    } catch (err) {
      ctx.json({ error: (err as Error).message }, 400);
    }
  ...
  /** Live-probe a linked node's agent: fetch /health + /info on demand and
   *  return ok + round-trip latency (or the fetch error). One-shot — does not
   *  mutate the registry. The admin "Test" button calls this. */
  router.post('/admin/cluster/test', async (ctx) => {
    const { id } = (ctx.body ?? {}) as { id?: string };
    if (!id) { ctx.json({ error: 'id is required' }, 400); return; }
    const client = manager.clientFor(id);
    if (!client) { ctx.json({ error: `node ${id} not linked` }, 404); return; }
    const t0 = Date.now();
    try {
      ...
      ctx.json({ ok: true, latencyMs, health, identity });
    } catch (err) {
      const latencyMs = Date.now() - t0;
      ctx.json({ ok: false, latencyMs, error: (err as Error).message });
    }
  }, guard);

  /** Toggle a single node's `enabled` flag (the admin per-node switch). Persists
   *  to the registry file so it survives restarts, then re-polls the node so the
   *  overview reflects the new state immediately. */
  router.post('/admin/cluster/enable', async (ctx) => {
    const { id, enabled } = (ctx.body ?? {}) as { id?: string; enabled?: boolean };
    if (!id || typeof enabled !== 'boolean') { ctx.json({ error: 'id + enabled (boolean) are required' }, 400); return; }
    try {
      await manager.setEnabled(id, enabled);
      ctx.json({ ok: true, id, enabled });
    } catch (err) {
      ctx.json({ error: (err as Error).message }, 400);
    }
  }, guard);
