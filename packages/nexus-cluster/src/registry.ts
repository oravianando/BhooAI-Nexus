...
export class NodeRegistry {
  ...
  /** Flip a node's `enabled` flag and persist it to the registry file so the
   *  change survives restarts (used by the admin per-node enable/disable toggle
   *  and by scaleTo). Returns false if the node isn't linked. */
