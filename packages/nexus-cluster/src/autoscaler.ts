...
export class Autoscaler {
  ...
  async tick(): Promise<DeterministicDecision> {
    ...
    if (backendNodes.length === 0) {
      await this.scale('scaleUp', 'no backend nodes in registry');
    ...
    if (backendNodes.length < this.cfg.maxNodes && input.perNodeRps > this.cfg.rpsPerNodeHigh) {
      await this.scale('scaleUp', `rps/node ${input.perNodeRps.toFixed(1)} > ${this.cfg.rpsPerNodeHigh}`);
    ...
    if (input.cpu !== null && input.cpu > this.cfg.cpuHigh && backendNodes.length < this.cfg.maxNodes) {
      await this.scale('scaleUp', `cpu ${input.cpu.toFixed(1)}% > ${this.cfg.cpuHigh}`);
      this.lastDecisionAt = now;
    ...
    if (backendNodes.length > this.cfg.minNodes && input.perNodeRps < this.cfg.rpsPerNodeLow) {
      await this.scale('scaleDown', `rps/node ${input.perNodeRps.toFixed(1)} < ${this.cfg.rpsPerNodeLow}`);
      this.lastDecisionAt = now;
  ...
  /** Run a scale operation, swallowing node-agent failures so a dead node never crashes the process. */
  private async scale(
    dir: 'scaleUp' | 'scaleDown',
    reason: string,
  ): Promise<void> {
    const fn = this.deps[dir];
    if (!fn) return;
    try {
      await fn(reason);
    } catch (err) {
      console.error(`[cluster] autoscaler ${dir} failed (${reason}): ${(err as Error).message}`);
    }
  }
  ...
  start(intervalMs = 10_000): void {
    ...
    const loop = () => {
      if (!this.running) return;
      void this.tick()
        .catch((err) => console.error(`[cluster] autoscaler tick failed: ${(err as Error).message}`))
