...
import {
  ...
  getClusterOverview, clusterLink, clusterUnlink, clusterExec, clusterScale, clusterPoll, clusterTest, clusterEnable, clusterNodeRole, getClusterSetup, saveClusterSetup, clusterStart, clusterStop, generateClusterToken, getClusterNodeStatus, clusterNodeStart, clusterNodeStop,
  ...
  type ServiceState, type AdminConfig, type AdminEnv, type EnvEntry, type DatabaseInfo, type GeneratedSchema, type PaymentProviderStatus, type PaymentProviderField, type AiStatus, type AiModelInfo, type AiChatMessage, type AiProviderView, type AiProviderPersistence, type AiProviderTestResult, type UserRecord, type RoleDefinition, type PreflightReport, type PreflightCheck, type LintReport, type ClusterNodeView, type ClusterOverview, type ClusterSetup, type ClusterNodeAgentStatus, type ClusterTestResult, type ClusterPathPin,
} from './api.js';
...
function Cluster() {
  ...
  const [testResults, setTestResults] = useState<Record<string, ClusterTestResult | undefined>>({});
  const [pathPins, setPathPins] = useState<ClusterPathPin[]>([]);
  /** True while the operator has unsaved path-pin edits — stops the 5s overview
   *  refresh from clobbering the editor mid-typing. Cleared on save. */
  ...
  const refresh = async () => {
    try {
      if (livePoll) { try { await clusterPoll(); } catch { /* best-effort */ } }
      const o = await getClusterOverview();
      setOverview(o);
      // Don't clobber unsaved path-pin edits with the server snapshot.
      if (!pinsDirtyRef.current) {
        // Prune pins whose node was unlinked so the editor never shows stale pins.
        const linkedIds = new Set((o.nodes ?? []).map((n) => n.id));
        const serverPins = o.pathPins ?? [];
        setPathPins(serverPins.filter((p) => linkedIds.has(p.nodeId)));
      }
      setMsg('');
  ...
  /** Live-probe a node's agent and show ok/latency inline. Doesn't go through the
   *  global `busy` guard so it can run per-row without locking the whole page. */
  const testNode = async (id: string) => {
    setTestResults((m) => ({ ...m, [id]: { ok: false, latencyMs: 0, error: '…' } }));
    try {
      const r = await clusterTest(id);
      setTestResults((m) => ({ ...m, [id]: r }));
    } catch (e: any) {
      setTestResults((m) => ({ ...m, [id]: { ok: false, error: String(e?.message ?? e) } }));
    }
  };
  /** Flip a node's enabled flag (the per-row switch), then refresh the overview. */
  const toggleEnabled = (id: string, enabled: boolean) => run(async () => {
    const r = await clusterEnable(id, enabled);
    setOverview(await getClusterOverview());
    return r;
  });
  /** Force a one-shot poll of all nodes, then refresh. */
  const pollNow = () => run(async () => { await clusterPoll(); setOverview(await getClusterOverview()); });
  ...
  /** Change this node's cluster role: persist + restart the agent, then refresh
   *  status + setup so the displayed node id / serve command reflect the new role. */
  const applyRole = (role: 'backend' | 'files' | 'database' | 'ai') => run(async () => {
    const r = await clusterNodeRole(role);
    setNodeStatus(await getClusterNodeStatus());
    setSetup(await getClusterSetup());
    return r;
  });

  /** Path-routing editor helpers (master mode): add/remove pins + save.
   *  Removals auto-persist immediately (destructive — the user expects the pin
   *  gone from the server right away, not lingering until a manual Save). Adds
   *  + edits mark dirty so the 5s refresh doesn't clobber mid-typing; removal
   *  clears dirty and saves, then resyncs from the server. */
  const addPathPin = () => { pinsDirtyRef.current = true; setPathPins((p) => [...p, { prefix: '/', nodeId: overview?.nodes[0]?.id ?? '', role: undefined }]); };
  const updatePathPin = (i: number, patch: Partial<ClusterPathPin>) => { pinsDirtyRef.current = true; setPathPins((p) => p.map((pin, idx) => idx === i ? { ...pin, ...patch } : pin)); };
  const removePathPin = (i: number) => {
    const next = pathPins.filter((_, idx) => idx !== i);
    setPathPins(next);
    pinsDirtyRef.current = false;
    void run(async () => {
      const r = await saveClusterSetup({}, next);
      if (!r?.error) setOverview(await getClusterOverview());
      return r;
    });
  };
  const savePathPins = () => run(async () => {
    const r = await saveClusterSetup({}, pathPins);
    if (!r?.error) { pinsDirtyRef.current = false; setOverview(await getClusterOverview()); }
    return r;
  });
    ...
    <div className="space-y-5">
      ...
      {mode === 'master' ? (
        <>
          ...
          {/* linked nodes — aggregate header + per-node cards */}
          {(() => {
            const nodes = overview?.nodes ?? [];
            const totalRps = nodes.reduce((a, n) => a + (typeof n.rps === 'number' ? n.rps : 0), 0);
            const readyCount = nodes.filter((n) => n.status === 'ready' && n.enabled).length;
            const maxMem = Math.max(1, ...nodes.map((n) => n.metrics?.memoryMb ?? 0));
            const allocPct = (n: ClusterNodeView) => (totalRps > 0 ? ((n.rps ?? 0) / totalRps) * 100 : 0);
            const nodeColor = (id: string) => {
              const palette = ['#34d399', '#60a5fa', '#f472b6', '#fbbf24', '#a78bfa', '#22d3ee', '#fb7185', '#4ade80'];
              let h = 0; for (let i = 0; i < id.length; i++) h = (h * 31 + id.charCodeAt(i)) >>> 0;
              return palette[h % palette.length];
            };
            const svcKeys = ['backend', 'files', 'database', 'ai'];
            const stale = (ms?: number) => (typeof ms === 'number' ? Date.now() - ms > 30_000 : true);
              ...
              <>
                ...
                <div className="glass-card">
                  <div className="flex flex-wrap items-end justify-between gap-3">
                    <div>
                      <span className="admin-overline">CONNECTED NODES</span>
                      <div className="mt-1 flex items-baseline gap-4">
                        <h3 className="glass-h3">{nodes.length} node{nodes.length !== 1 ? 's' : ''}</h3>
                        <span className="text-sm text-emerald-300">{readyCount} ready</span>
                        <span className="text-sm text-slate-400">{totalRps.toFixed(1)} rps total</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <label className="flex items-center gap-1.5 text-xs text-slate-400">
                        <input type="checkbox" checked={livePoll} onChange={(e) => setLivePoll(e.target.checked)} className="h-3.5 w-3.5" />
                        live metrics
                      </label>
                      <button onClick={pollNow} disabled={busy} className="glass-chip-btn">↻ poll now</button>
                    </div>
                  </div>
                  {totalRps > 0 && (
                    <div className="alloc-split mt-3" title={`allocation across ${nodes.length} node(s)`}>
                      {nodes.map((n) => {
                        const p = allocPct(n);
                        return p > 0 ? (
                          <div key={n.id} className="alloc-split-seg" style={{ width: `${p}%`, background: nodeColor(n.id) }} title={`${n.id} · ${p.toFixed(1)}%`} />
                        ) : null;
                      })}
                    </div>
                  )}
                ...
                {/* per-node cards */}
                {nodes.length === 0 && (
                  <div className="glass-panel p-8 text-center text-slate-500">No nodes linked yet. Paste a node's agent URL above to register it.</div>
                )}
                <div className="grid gap-3 lg:grid-cols-2">
                  {nodes.map((n) => {
                    const p = allocPct(n);
                    const cpu = n.metrics?.cpu ?? null;
                    const mem = n.metrics?.memoryMb ?? null;
                    const t = testResults[n.id];
                    const svc = n.health?.services ?? {};
                    return (
                      <div key={n.id} className={`node-card ${n.status === 'unreachable' ? 'is-bad' : n.status === 'ready' ? 'is-ok' : 'is-warn'}`}>
                        {/* header */}
                        <div className="flex items-center gap-2">
                          <span className={`inline-block h-2.5 w-2.5 rounded-full ${n.status === 'ready' ? 'bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.9)]' : n.status === 'unreachable' ? 'bg-rose-500' : 'bg-amber-400'}`} />
                          <span className="font-mono text-sm text-slate-100 truncate">{n.id}</span>
                          <span className="glass-chip">{n.role}</span>
                          {n.tier && <span className="glass-chip text-slate-400">{n.tier}</span>}
                          {stale(n.metrics?.sampledAt) && n.status === 'ready' && <span className="freshness-badge freshness-stale">stale</span>}
                          <span className="ml-auto flex items-center gap-1.5">
                            <span className={`toggle-switch ${n.enabled ? 'is-on' : ''}`} title={n.enabled ? 'enabled (in rotation)' : 'standby'}>
                              <input type="checkbox" checked={n.enabled} onChange={(e) => toggleEnabled(n.id, e.target.checked)} disabled={busy} />
                              <span className="toggle-knob" />
                            </span>
                            <span className="text-xs text-slate-400">{n.enabled ? 'enabled' : 'standby'}</span>
                          </span>
                        </div>
                        ...
                        <div className="mt-3">
                          <div className="flex items-baseline justify-between">
                            <span className="admin-overline">REQUESTS HANDLED</span>
                            <span className="text-xs text-slate-400">{(n.rps ?? 0).toFixed(1)} rps · <span className="text-slate-200">{p.toFixed(1)}%</span></span>
                          </div>
                          <div className="alloc-bar mt-1">
                            <div className={`alloc-bar-fill ${n.status === 'ready' && n.enabled ? 'is-ready' : 'is-idle'}`} style={{ width: `${Math.max(2, p)}%`, background: n.status === 'ready' && n.enabled ? nodeColor(n.id) : undefined }} />
                          </div>
                          <p className="mt-1 text-[10px] text-slate-500">share of {totalRps.toFixed(1)} rps via the load balancer · excludes /auth (pinned to master)</p>
                        </div>

                        {/* load row: cpu + mem */}
                        <div className="mt-3 grid grid-cols-2 gap-3">
                          <div>
                            <div className="flex items-baseline justify-between"><span className="admin-overline">CPU</span><span className="text-xs text-slate-400">{cpu !== null ? `${cpu.toFixed(1)}%` : '—'}</span></div>
                            <div className="load-bar mt-1">
                              <div className="load-bar-fill" style={{ width: `${Math.min(100, cpu ?? 0)}%`, background: (cpu ?? 0) > 80 ? '#fb7185' : (cpu ?? 0) > 60 ? '#fbbf24' : '#34d399' }} />
                            </div>
                          </div>
                          <div>
                            <div className="flex items-baseline justify-between"><span className="admin-overline">MEM</span><span className="text-xs text-slate-400">{mem !== null ? `${mem} MiB` : '—'}</span></div>
                            <div className="load-bar mt-1">
                              <div className="load-bar-fill" style={{ width: `${mem ? Math.min(100, (mem / maxMem) * 100) : 0}%`, background: '#60a5fa' }} />
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="mt-3 flex items-center gap-3">
                          <span className="admin-overline">SERVICES</span>
                          <div className="flex items-center gap-1.5">
                            {svcKeys.map((k) => {
                              const v = svc[k];
                              return (
                                <span key={k} className={`svc-dot ${v === true ? 'is-ok' : v === false ? 'is-bad' : 'is-unknown'}`} title={`${k}: ${v === true ? 'ok' : v === false ? 'down' : 'n/a'}`} />
                              );
                            })}
                          </div>
                          <span className="ml-auto text-[10px] text-slate-500">
                            {n.lastSeenAt ? `seen ${new Date(n.lastSeenAt).toLocaleTimeString()}` : ''}
                          </span>
                        </div>
                        ...
                        <div className="mt-2 flex items-center gap-2 text-[10px] text-slate-500">
                          ...
                          {n.serviceUrl && <code className="truncate">{n.serviceUrl}</code>}
                        </div>

                        {/* controls */}
                        <div className="mt-3 flex flex-wrap items-center gap-1.5 border-t border-slate-700/50 pt-3">
                          <button onClick={() => testNode(n.id)} disabled={busy} className="glass-chip-btn">test</button>
                          {t && (
                            <span className={`text-[11px] ${t.ok ? 'text-emerald-300' : 'text-rose-300'}`}>
                              {t.ok ? `✓ ${t.latencyMs ?? 0}ms` : `✗ ${t.error ?? 'failed'}`}
                            </span>
                          )}
                          ...
                          <button onClick={() => unlink(n.id)} disabled={busy} className="glass-chip-btn ml-auto">unlink</button>
                        </div>
                      </div>
                    );
                  })}
          ...
          {/* Path routing — pin URL prefixes to specific nodes (upload-path allocation) */}
          <div className="glass-card space-y-3">
            <div className="flex items-end justify-between gap-3">
              <div>
                <span className="admin-overline">PATH ROUTING</span>
                <h3 className="glass-h3">Pin URL prefixes to nodes</h3>
                <p className="mt-1 text-xs text-slate-400">
                  Route a path prefix to a specific node. If that node is down, the LB returns 503 (no fallback).
                  Example: <code className="text-emerald-300">/upload/users</code> → slave-1, <code className="text-emerald-300">/upload/files</code> → slave-2.
                </p>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={addPathPin} disabled={busy} className="glass-chip-btn">+ add pin</button>
                <button onClick={savePathPins} disabled={busy || pathPins.length === 0} className="glass-btn-primary">Save routing</button>
              </div>
            </div>

            {pathPins.length === 0 ? (
              <div className="rounded-lg border border-slate-700/60 bg-slate-900/40 p-4 text-center text-sm text-slate-500">
                No path pins — all traffic round-robins across ready backend nodes.
              </div>
            ) : (
              <div className="space-y-2">
                {pathPins.map((pin, i) => {
                  const matched = overview?.nodes.find((n) => n.id === pin.nodeId);
                  const stale = matched && matched.status !== 'ready';
                  return (
                    <div key={i} className="path-pin-row">
                      <input
                        value={pin.prefix}
                        onChange={(e) => updatePathPin(i, { prefix: e.target.value })}
                        placeholder="/upload/users"
                        className="glass-input path-pin-prefix font-mono"
                      />
                      <span className="path-pin-arrow">→</span>
                      <select
                        value={pin.nodeId}
                        onChange={(e) => updatePathPin(i, { nodeId: e.target.value })}
                        className="glass-input path-pin-node"
                      >
                      ...
                      <select
                        value={pin.role ?? ''}
                        onChange={(e) => updatePathPin(i, { role: (e.target.value || undefined) as ClusterPathPin['role'] })}
                        className="glass-input path-pin-role"
                      >
                      ...
                      {stale && <span className="freshness-badge freshness-stale">node down</span>}
                      <button onClick={() => removePathPin(i)} disabled={busy} className="glass-chip-btn-danger path-pin-del">✕</button>
                    </div>
                  );
                })}
              </div>
        ...
        <>
          ...
          <div className="glass-card space-y-4">
            <div>
              ...
              <p className="text-sm text-slate-400">Pick this node's role, start the agent, then share its URL, node id, and token with the master operator.</p>
            </div>

            {/* Role picker */}
            <div>
              <span className="admin-overline">ROLE</span>
              <div className="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                {([
                  { r: 'backend', icon: '⚙', label: 'backend', desc: 'API core' },
                  { r: 'files', icon: '📂', label: 'files', desc: 'Static + uploads' },
                  { r: 'database', icon: '🗄', label: 'database', desc: 'Mongo + Redis' },
                  { r: 'ai', icon: '🤖', label: 'ai', desc: 'AI inference' },
                ] as const).map((opt) => {
                  const active = (nodeStatus?.role ?? 'backend') === opt.r;
                  return (
                    <button
                      key={opt.r}
                      onClick={() => applyRole(opt.r)}
                      disabled={busy}
              ...
              <p className="mt-2 text-[11px] text-amber-300/80">
                Changing the role rewrites the node id — re-link this node on the master if it was already linked.
              </p>
            </div>
            ...
            <div className="rounded-lg border border-slate-700/60 bg-slate-900/40 p-3">
              <div className="flex items-center gap-2">
                <span className={`inline-block h-2 w-2 rounded-full ${nodeStatus?.running ? 'bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.9)]' : 'bg-slate-500'}`} />
                <span className="text-sm text-slate-200">{nodeStatus?.running ? 'Agent running' : 'Agent stopped'}</span>
                {nodeStatus?.pid && <span className="text-[11px] text-slate-500">pid {nodeStatus.pid}</span>}
              </div>
            </div>

            {/* Identity + URL */}
            <div className="grid gap-2 sm:grid-cols-2 rounded-lg border border-slate-700/60 bg-slate-900/40 p-3 font-mono text-xs">
              <div>
                <span className="block text-slate-500">node id</span>
                <span className="text-slate-200 break-all">{nodeStatus?.nodeId ?? '—'}</span>
              </div>
              <div>
                <span className="block text-slate-500">role · port</span>
                <span className="text-slate-200">{nodeStatus?.role ?? 'backend'} · {nodeStatus?.port ?? setup?.self?.port ?? '—'}</span>
              </div>
              <div className="sm:col-span-2">
                <span className="block text-slate-500">agent URL — give this to the master</span>
                <div className="mt-1 flex items-center gap-2">
                  <code className="flex-1 truncate text-emerald-300">{setup?.self?.agentUrl ?? nodeStatus?.agentUrl ?? '—'}</code>
                  <button disabled={busy} className="glass-chip-btn shrink-0 text-[10px]" onClick={() => copyText(setup?.self?.agentUrl ?? nodeStatus?.agentUrl ?? '')}>copy</button>
                </div>
              </div>
            </div>
            ...
            {/* Quick-start commands (role-aware) */}
            <div className="rounded-lg border border-slate-700/60 bg-slate-900/40 p-3 space-y-1 font-mono text-xs">
              <div className="flex items-center gap-2">
                <span className="text-slate-500 shrink-0 w-12">serve</span>
                <code className="flex-1 truncate text-slate-200">{setup?.self?.serve ?? '—'}</code>
                <button disabled={busy} className="glass-chip-btn shrink-0 text-[10px]" onClick={() => copyText(setup?.self?.serve ?? '')}>copy</button>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-slate-500 shrink-0 w-12">link</span>
                <code className="flex-1 truncate text-slate-200">{setup?.self?.link ?? '—'}</code>
                <button disabled={busy} className="glass-chip-btn shrink-0 text-[10px]" onClick={() => copyText(setup?.self?.link ?? '')}>copy</button>
              </div>
            </div>
