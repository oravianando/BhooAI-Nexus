...
export interface AdminConfig { runtime: Record<string, unknown>; config: Record<string, unknown>; file: ConfigFile | null; }
export interface EnvEntry { key: string; value: string | null; secret: boolean; configPath?: string; }
export interface AdminEnv { path: string; exists: boolean; entries: EnvEntry[]; note?: string; }
...
export const getAdminEnv = (): Promise<AdminEnv> => adminFetch('/admin/env');
export async function putAdminEnv(entries: EnvEntry[]): Promise<any> {
  await refreshCsrf();
  return adminFetch('/admin/env', { method: 'PUT', headers: { 'content-type': 'application/json', 'x-csrf-token': csrfToken }, body: JSON.stringify({ entries }) });
}
