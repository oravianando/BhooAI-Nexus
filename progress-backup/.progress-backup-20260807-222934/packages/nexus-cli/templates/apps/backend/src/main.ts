...
async function main(): Promise<void> {
  ...
  // Connect to MongoDB (the ODM registers models on the default connection).
  connect(config.db.uri, { name: config.db.name, autoIndex: config.db.autoIndex });
  await getConnection().db;
  log.info('connected to MongoDB', { db: config.db.name ?? 'from-uri' });
  ...
  registerAiRoutes(router, { serverUrl: config.ai.serverUrl, timeoutMs: config.ai.timeoutMs });
  ...
  registerAdminRoutes(router, config, { root: PROJECT_ROOT, adminExtensions: plugins.adminExtensions, metrics: () => metrics.toJSON(), payments });
