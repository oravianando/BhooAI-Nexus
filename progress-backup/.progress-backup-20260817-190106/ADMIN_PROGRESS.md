...
## Rail + notification polish (2026-08-13, continued)
- Removed the rail ◍ Notifications button entirely (notifications live only on the top-bar ⌁);
  rail now has Smart search (⌕), AI Notes (▣), Screen recall (◫).
- FIXED: clicking AI Notes / Screen recall closed the panel immediately — the rail popover rendered
  outside `railRef`, so the outside-click handler closed it on first click inside. Wrapped the
  `RailPanel` render in a `railPanelRef` div and included it in the outside-click guard. Also
  `RailPanel` now returns null for `open === 'notify'` so ⌁ opens only `NotifyPanel` (no empty
  `rail-panel` div). Both popovers set `aria-modal="false"`.
- AI Schema page: added `mr-1.5` gap after the "Model" and "Provider" labels (select/input spacing).
- Z-index layering: `.admin-search-results` → `9999` (above all); `.admin-topbar` → `position:
  relative; z-index: 10` (lifts header stacking context above content so the dropdown paints in
  front); `.overview-stat-grid` → `position: relative; z-index: 1`.
- Dropdown carets centered under their trigger buttons: `.appearance-panel::before` → `right:
  9.5px` (30px ◒ button), `.account-panel::before` → `right: 10px` (31px avatar).
- FIXED: notify caret was invisible — `.notify-panel { overflow: hidden }` was clipping the `::before`
  caret; removed it and gave `.notify-head` `border-radius: 0.85rem 0.85rem 0 0`; caret now shows
  centered at `right: 9.5px` under the ⌁ button.
- Files: `packages/nexus-admin/src/App.tsx` + `index.css`. No backend changes. Build passes.
