...
export class RealtimeServer {
  ...
  private async handleUpgrade(req: IncomingMessage, socket: import('node:net').Socket, head: Buffer): Promise<void> {
  ...
  private relayFrom(conn: Connection, msg: ClientMessage): ServerMessage {
    ...
    return { type: 'candidate', from: conn.id, candidate: msg.candidate };
  }
