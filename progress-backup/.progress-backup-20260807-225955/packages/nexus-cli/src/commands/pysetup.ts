...
interface PyOpts { python: string; requirements: string; extras: string[]; upgrade: boolean; venv: boolean; venvDir: string }
...
function parseArgs(args: string[]): PyOpts {
  ...
  return { python, requirements, extras, upgrade, venv, venvDir: join(reqDir, '.venv') };
}
