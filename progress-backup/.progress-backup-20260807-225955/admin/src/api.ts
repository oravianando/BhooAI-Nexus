...
export interface PreflightCheck { name: string; kind: string; ok: boolean; latencyMs?: number; status?: number | null; url?: string; host?: string; error?: string; }
export interface PreflightReport { ranAt: string; durationMs: number; passed: number; warnings: number; failed: number; checks: PreflightCheck[]; }
