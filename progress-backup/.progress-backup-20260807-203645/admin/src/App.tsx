...
import {
  ...
  type ServiceState, type AdminConfig, type AdminEnv, type EnvEntry, type DatabaseInfo, type GeneratedSchema, type PaymentProviderStatus, type AiStatus, type UserRecord, type RoleDefinition,
} from './api.js';
...
function Dashboard({ user, onLogout }: { user: AdminUser | null; onLogout: () => void }) {
  ...
  const shellStyle: React.CSSProperties = { ['--admin-shell-width' as string]: `${shellWidth}px` };
  if (customTheme.active) {
    Object.assign(shellStyle, {
      ...
      background: `radial-gradient(60rem 60rem at 85% -10%, color-mix(in srgb, ${customTheme.violet} 42%, transparent), transparent 70%), radial-gradient(50rem 50rem at -10% 110%, color-mix(in srgb, ${customTheme.blue} 36%, transparent), transparent 70%), radial-gradient(45rem 45rem at 50% 50%, color-mix(in srgb, ${customTheme.pink} 22%, transparent), transparent 70%), ${customTheme.bg}`,
    });
  }
  return (
    <div className={`admin-shell bg-mesh theme-${theme} ui-${uiMode}`} style={shellStyle}>
      <div className="admin-particles" aria-hidden="true"><i /><i /><i /><i /><i /><i /><i /><i /><i /><i /><i /><i /></div>
      ...
      <div className="admin-main-shell">
        ...
        <div className="admin-content-layout">
          <main className="admin-content">
            ...
            {tab === 'schema' && <Schema />}
            {tab === 'theme' && <ThemeCentre theme={theme} onThemeChange={(value) => { setTheme(value); window.localStorage.setItem('nexus-admin-theme', value); }} uiMode={uiMode} onUiModeChange={(value) => { setUiMode(value); window.localStorage.setItem('nexus-admin-ui', value); }} custom={customTheme} onCustom={updateCustomTheme} />}
          </main>
  ...
  return (
    <div className="space-y-6">
      <PageHead title="Theme Centre">
        <button type="button" className="workspace-action" onClick={() => { onCustom({ ...THEME_PRESETS[theme], active: false }); try { window.localStorage.removeItem(CUSTOM_THEME_KEY); } catch { /* ignore */ } }}>↻ Reset</button>
      </PageHead>
      <PageHero kicker="SURFACE STUDIO" title={<>Make it <em>yours.</em></>} desc="Curated themes or your own mix of accent and canvas colors — applied live to this workspace and remembered locally in your browser." glyph="◒" art="palette" />
        ...
        <div className="custom-grid">
          {colors.map((c) => (
            <label key={c.key} className="color-row">
              <span className="color-swatch"><input type="color" value={custom[c.key]} disabled={!custom.active} onChange={(e) => onCustom({ [c.key]: e.target.value })} /></span>
              <span className="color-meta"><b>{c.label}</b><small>{c.detail}</small></span>
              <input className="glass-input color-hex" value={custom[c.key]} disabled={!custom.active} onChange={(e) => { const v = e.target.value.trim(); if (/^#[0-9a-fA-F]{3,8}$/.test(v)) onCustom({ [c.key]: v }); }} />
            </label>
          ))}
        </div>
      </section>
